...
$TotalComTit = "Total Commander (x64) 9.0a - Freemen 17.1 lite"
$PdfViewer = "[CLASS:DSUI:PDFXCViewer]"
$DjvuViewer = "[CLASS:SUMATRA_PDF_FRAME]"
$GenPath = "z:\_Для прикрепа\Новь\Делаются о. Павлом\"
$spec = ""
$invNum = ""
...
Func OpenFile()
	WinActivate($TotalComTit)
	If WinWaitActive($TotalComTit, "", 3) Then
...
;~ 		Get file name and parse
		...
		Send("{F2 2}")
		$clip = ""
		While (StringInStr($clip, ".djvu") == 0)
			Send("^c")
			Sleep(100)
			$clip = ClipGet()
		WEnd
		$invNum = ""
		$invNum = StringSplit($clip, " ")[1]
		$InvNumExist = StringIsInt($invNum)
		If $InvNumExist And StringLen($invNum) < 7 Then
			Sleep(200)
			Send("{F2}")
			Send("{HOME}{SHIFTDOWN}" & "^{RIGHT}" & "{SHIFTUP}")
			Send("{DELETE}")
		EndIf
		$fulltext = StringSplit($clip, " ")[1]
...
;~ 		MsgBox(0, "", $clip)
	...
	If WinWaitActive($PdfViewer, "", 5) Or WinWaitActive($DjvuViewer, "", 5) Then
		If StringIsInt($fulltext) Then
			OpenByFulltext($fulltext)
		ElseIf $InvNumExist Then
			OpenByInv($invNum)
		Else
			OpenByTitle($clip)
		EndIf
...
Func OpenByFulltext($fulltext)
	...
	If WinWaitActive($IrbisTit, "", 3) Then
		...
		If StringInStr($searchTherm, "951") == 0 Then
			Srchfor("951")
		Else
			Send("!d" & "!d")
		EndIf
		ClipPut("\\FULLTEXT\\" & $fulltext)
		Send("+{INSERT}")
		Sleep(100)
		Send("{ENTER}")
		Sleep(100)
		...
		Send("!d" & "!d{UP}")
	EndIf
EndFunc   ;==>OpenByFulltext

Func OpenByInv($invNum)
	WinActivate($IrbisTit)
	If WinWaitActive($IrbisTit, "", 3) Then
		Sleep(100)
		OptimizeView()
...
Func OpenByTitle($clip)
	...
	If WinWaitActive($IrbisTit, "", 3) Then
		...
		Sleep(100)
		$searchTherm = ControlGetText($IrbisTit, "", "[CLASS:THSHintComboBox]")
		If StringInStr($searchTherm, "Заглавие") == 0 Then
			Srchfor("заг")
		Else
			Send("!d" & "!d")
		EndIf
...
Func MoveOrRename()
	If WinGetHandle("[ACTIVE]") <> WinGetHandle($TotalComTit) Then
		MoveFile()
	Else
		$panelText1 = ControlGetText($TotalComTit, "", "[CLASS:Window; INSTANCE:12]")
		$panelText2 = ControlGetText($TotalComTit, "", "[CLASS:Window; INSTANCE:17]")
...
;~ 			ClipPut($GenPath & $folder & "\")
			Send("!{F2}" & "z")
			If StringInStr($GenPath, "z:\_Для прикрепа\Новь\") > 0 And StringLen($GenPath) > StringLen("z:\_Для прикрепа\Новь\") Then
				Send("{BACKSPACE}")
			EndIf
			Send($folder)
			Send("{ENTER}")
...
;~ 		search underscore
				...
				Else
					Send("{RIGHT}")
				EndIf
;~ 		delete fulltext number
				If $folder <> "э" Then
					Send("{SHIFTDOWN}" & "^{RIGHT}" & "{LEFT}" & "{SHIFTUP}" & "^c")
					$int = ClipGet()
